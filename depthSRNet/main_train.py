...
parser = argparse.ArgumentParser(description='DEPTH-SRNET')
...
parser.add_argument('--resize_num', type=int, required=True, help='resize size')
...
train_dataset = StereoDataset(args.datapath, args.trainlist, True, args.train_crop_height, args.train_crop_width, args.channels, args.resize_num)
...
model = __models__[args.model](10, 10, 8, 8)
...
# loss: new BCE loss
# loss_back = nn.BCEWithLogitsLoss()
...
def train_sample(sample, compute_metrics=False):
    ...
    # training data load
    lr, sr = sample['img_lr'], sample['img_sr']
    lr = lr.cuda()
    sr = sr.cuda()
    #
    optimizer.zero_grad()
    #
    pre = model(lr)
    # disp_gt = disp_gt.float()  # byte tensor -- float tensor
    loss = loss_back(pre, sr)
    scalar_outputs = {"loss": loss}
    image_outputs = {"LR": lr, "SR": sr, "pre_LR": pre}
